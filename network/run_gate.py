...
if config["use_dummy"]:
    x = torch.randn(5, 4)
    edge_index = torch.tensor([[0, 1, 2, 3, 4, 1, 2], [1, 0, 3, 4, 3, 2, 1]])
    ...
    if remove_rt_thresh:
        ...
        mask = ~(
            (df_nodes["num_post"] == 0) & (df_nodes["user_rt"] <= remove_rt_thresh)
        )
        ...
        df_G = df_G[
            df_G["source"].isin(valid_nodes) & df_G["target"].isin(valid_nodes)
    ...
    edge_index = torch.tensor(
        [
            [node2idx[src] for src in df_G["source"]],
            [node2idx[tgt] for tgt in df_G["target"]],
        ],
        dtype=torch.long,
    )
    structure_pairs = edge_index
...
for epoch in range(epochs):
    ...
    if epoch % 10 == 0:
        print(
            f"Epoch {epoch:3d} | Loss: {total_loss:12.4f} | Feature Loss: {feature_loss:12.4f} | Structure Loss: {structure_loss:12.4f}"
        )
