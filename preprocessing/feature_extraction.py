"""
Extracts the features from the dataset
"""
...
def tweet_pipeline(tweet):
    ...
    num_all_caps = (
        len([word for word in words if word.isupper()]) - num_urls
    )  # to not count URL
    num_emoji = len([char for char in tweet if char in emoji.EMOJI_DATA])
    num_punc = (
        sum(1 for char in tweet if char in string.punctuation)
        - num_hashtags
        - num_mentions
    )  # to not count mentions and hashtags
...
tweets_df = pd.read_csv(
    PATH + "source_tweets.txt", sep="\t", header=None, names=["tweet_id", "tweet"]
)
labels_df = pd.read_csv(
    PATH + "label.txt", sep=":", header=None, names=["label", "tweet_id"]
)
